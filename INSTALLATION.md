...
Project Asylum artik tum desteklenen isletim sistemlerinde ortak bir `customer mode` hedefiyle dusunulur:
- `linux`
- `macos`
- `windows`

Customer mode amaci:
- gelistirici kurulumu ile musteri kurulumunu ayirmak,
- Node/npm eksikligi gibi sorunlari kurulumun en basinda yakalamak,
- ileride bundled runtime ile tam paket kuruluma gecise zemin hazirlamak.

Bu modda ilk zorunlu adim `preflight` kontroludur.

```bash
npm run install:preflight
```

Preflight su bilgileri raporlar:
- isletim sistemi ailesi
- Node surumu
- npm durumu
- customer mode kuruluma hazir olup olmadigi
- eksikse neden hazir olmadigi
- ilgili OS icin yonlendirme notlari
...
4. Bootstrap çalıştır:

```bash
npm run install:bootstrap
```

5. Kurulum doğrulamasını çalıştır:

```bash
npm run install:doctor
```

6. Post-install kontrolünü çalıştır:
...
## Tek Komutlu Customer Mode Scripleri

Platforma gore kullanilabilecek scripler:
...
Bu scriptler kuruluma baslamadan once:
- `node` var mi
- `npm` var mi

kontrol eder. Eksikse kurulum zincirine girmez ve acik hata mesaji verir.
...
Bugun customer mode hala `Node 20+` beklemektedir. Ancak hedef mimari:
- bundled runtime
- packaged installer
- internet bagimsiz USB kurulumu

yonundedir. Mevcut preflight katmani bu gecise temel olacak sekilde eklendi.
