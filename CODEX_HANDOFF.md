...
- Sibling app `/Users/bariskockar/Desktop/bilet-app` icine gecici bir Project Asylum koprusu eklendi.
- Amac mevcut `bilet-app` serveri uzerinden ayri calisan Asylum instance'ini test etmek; kalici mimari degil, temp entegrasyon.
- Eklenen yeni dosyalar:
  - `/Users/bariskockar/Desktop/bilet-app/lib/asylum-bridge.ts`
  - `/Users/bariskockar/Desktop/bilet-app/app/api/asylum/system-summary/route.ts`
  - `/Users/bariskockar/Desktop/bilet-app/app/api/asylum/cognitive-summary/route.ts`
  - `/Users/bariskockar/Desktop/bilet-app/app/api/asylum/execute/route.ts`
  - `/Users/bariskockar/Desktop/bilet-app/app/asylum/page.tsx`
  - `/Users/bariskockar/Desktop/bilet-app/app/asylum/asylum-bridge-client.tsx`
- Bu kopru `ASYLUM_BASE_URL` (fallback `NEXT_PUBLIC_ASYLUM_BASE_URL`, sonra `http://localhost:3002`) uzerinden mevcut Asylum serverina proxy olur.
- `bilet-app/asylum` sayfasi sunar:
  - system summary
  - cognitive summary
  - prompt execute formu
  - mevcut Asylum UI'inin iframe embed'i
- Onemli sinir: mevcut `bilet-app` dosyalarinin cogu `compressed,dataless` iCloud placeholder durumunda oldugu icin (`Operation timed out` ile okunamadi), entegrasyon var olan `page.tsx` / `admin/page.tsx` icine gomulmek yerine izole bir temp route olarak eklendi.
...
- customer mode artık bütün OS'ler için ortak bir sözleşme olarak tanımlandı
- masaüstünde ayrı bir customer-mode delivery kit hazırlandı:
  - `/Users/bariskockar/Desktop/Project-Asylum-Customer-Mode-Kit`
  - içinde `release/customer-mode` bundle, kaynak kod kopyası, offline runtime klasörleri ve launcher iskeleti bulunuyor
- `release/customer-mode` içinde artık gerçek bundled runtime klasörleri var:
  - `windows x64/arm64`
  - `macos x64/arm64`
  - `linux x64/arm64`
- customer-mode bundle içinde:
  - `Project Asylum Installer.exe`
  - `Project Asylum Installer x64.exe`
  - `Project Asylum Installer.command`
  - `Project Asylum Open UI.command`
  - `Project Asylum Installer.app`
  yüzeyleri hazırlandı
- `release/customer-mode/scripts/install.sh` artık bundled runtime yolunu doğru nested klasörlerden arıyor:
  - `runtime/macos/node/<arch>/node-v20.20.2-.../bin/node`
  - `runtime/linux/node/<arch>/node-v20.20.2-.../bin/node`
  ve sistem `node` sadece fallback olarak kullanılıyor
- `release/customer-mode/scripts/install.sh` ve `install.ps1` artık kurulum scriptlerini bundle kökünde değil `app/` bağlamında çalıştırıyor; böylece `tsx` çözümlemesi `app/node_modules` üzerinden yapılabiliyor
- `release/customer-mode/scripts/launch-installed-app.mjs` eklendi:
  - `next start -p 3002` arka planda başlatır
  - server hazır olana kadar bekler
  - isterse tarayıcı açar
  - `release/app-launch.json` içine PID/URL yazar
- `desktop-launcher` Electron uygulaması çapraz platform hale getirildi:
  - bundled Node çözümlemesi artık `windows / macos / linux` için ayrı çalışıyor
  - installer akışı dış bundle klasörlerini `process.execPath` üzerinden buluyor
  - `Preflight` artık `app/scripts/install-preflight.ts` dosyasını `app/` çalışma dizininde çalıştırıyor
  - `Paneli Aç` artık tarayıcı sekmesi yerine ayrı Electron pencere açıyor
  - customer-mode preflight artık bundled `npm`'i de tespit ediyor; bu sayede bundle içindeki macOS runtime ile `readyForCustomerInstall=true` dönebiliyor
  - kritik düzeltme: bundled `npm` yolu bulunmasına rağmen çalışmıyordu; `install-preflight.ts` artık `execFileSync` ve PATH'e bundled Node dizinini ekleyerek `npmVersion` bilgisini doğru okuyabiliyor
  - sonraki regresyon turunda canonical ve bundle app `install-preflight.ts` tekrar esitlendi; bu sayede health karti `11/11` iken preflight'in `npm bulunamadi` diye yanlis-negatif vermesi kapandi
- bundle app içindeki kurulum UI daha yönlendirici hale getirildi:
  - install API ve summary route’ları dynamic/no-store oldu
  - başarılı adım sonrası sıradaki karta otomatik scroll eklendi
  - aktif adım vurgusu eklendi
  - kısa başarı toast ve kurulum tamamlandı callout eklendi
  - desktop launcher tarafında `Install Baslat` tamamlanınca artık `Paneli Aç` beklenmeden popup dashboard otomatik açılıyor
  - bunun için macOS `.app` yeniden paketlenip bundle köküne kopyalandı
- pilot kurulum sertlestirmesi için yeni dosyalar eklendi:
  - `/Users/bariskockar/Desktop/bilet-app/project asylum/PRODUCTION_READINESS_CHECKLIST.md`
  - `/Users/bariskockar/Desktop/bilet-app/project asylum/WINDOWS_VM_TEST_PLAN.md`
  - `/Users/bariskockar/Desktop/Project-Asylum-Customer-Mode-Kit/release/customer-mode/scripts/postinstall-self-check.mjs`
- `install.sh` artık `postcheck` sonrasında otomatik bir self-check raporu üretir:
  - `release/self-check-report.json`
  - bundled runtime, node_modules, .next, install-state ve observe-only kilidini kontrol eder
- pilot Windows sanal cihaz testi için bu Mac'e `UTM` kuruldu; sonraki blok resmi Windows ISO temini ve VM yaratimi
- bundle app installer yuzeyi guclendirildi:
  - `SystemSummary.installation.selfCheck` alani eklendi
  - installer ekraninda artik `Kurulum Sagligi` karti gorunuyor
  - `Tanilama indir` dugmesi `/api/install-diagnostics` uzerinden tek JSON raporu indiriyor
  - `postcheck` adimi app icindeki installer UI'dan da calistiginda self-check raporu otomatik uretiliyor
  - bundle app `next build` tekrar alindi; build gecti, sadece bilinen `lockfile missing swc dependencies` uyarisi kaldi
- desktop launcher UI da guclendirildi:
  - launcher artik `release/self-check-report.json` okuyup kendi icinde `Kurulum Sagligi` paneli gosteriyor
  - boylece musteri installer penceresinde self-check sonucunu browser dashboard'a gitmeden gorebiliyor
  - macOS `.app` yeniden paketlenip bundle kokune kopyalandi
  - launcher icine `Tanilamayi Masaustune Kaydet` dugmesi eklendi
  - bu dugme installer, bootstrap, self-check ve app-launch verilerini tek JSON dosyasi olarak masaustune export ediyor
  - macOS `.app` bu export akisi icin tekrar paketlenip guncellendi
  - launcher hero kartina `Pilot Hazir` rozeti eklendi
  - alt bolume `Musteri Teslim Ozeti` paneli eklendi
  - bu iki yuzey de desktop launcher'in ortak Electron kodunda oldugu icin Windows ve Linux hedefleriyle ayni feature set'e tasinacak
- agent omurgasi yeni `integrity` katmaniyla sertlestirildi:
  - yeni dosya: `/Users/bariskockar/Desktop/bilet-app/project asylum/src/lib/agent/integrity-engine.ts`
  - report icine `integrity` alani eklendi
  - bu alan `coverageScore`, `taskCompletionScore`, `evidenceScore`, `status`, `pilotReady`, `missingKinds` ve ozet metni tasir
  - amac kararin ne oldugunu degil, o karar icin kanit butunlugunun ne kadar saglam oldugunu da gostermek
  - son turda integrity ayrica `coherenceScore`, `contradictions`, `contradictionCount` tasir hale geldi
  - observation, risk, task ve decision zinciri celisirse bu durum `high-risk-marked-completed`, `risk-without-review-task` gibi kodlarla raporlanir
  - `pilotReady` artik contradiction olmayan ve coherence skoru yeterli execution'larla sinirlanir
  - son turda contradiction'lar siniflandirildi:
    - `evidence-gap`
    - `risk-decision-mismatch`
    - `task-flow-mismatch`
  - `integrity.contradictionGroups` artik bu kategorileri ve alt kalemleri tasir
  - `platform-data` son turda bu contradiction gruplarini insan-okur aciklamalara cevirir
  - `CognitiveSummary.integrity.contradictionExplanations` artik operator veya musteriye “neden temkinli kaldı” sorusunun dogrudan cevabini verir
  - hem canonical dashboard hem customer-mode dashboard bu aciklamalari `Kanıt` paneli altinda gosterir
- contradiction-aware mantik sonraki turda reasoning/critic/decision katmanlarina da tasindi
  - `reasoning-engine` contradiction gordugunde belief summary ve nextInference icinde bunu acikca yazar
  - `critic-engine` contradiction varsa `evidence-coherence-review` flag'i ekler ve summary/recommendedAction buna gore sertlesir
  - `decision-engine` contradiction varsa blocker ve rationale seviyesinde bunu gorunur kilir
- trust/autonomy gate artik integrity ile birlikte calisiyor:
  - `trust-engine` artik `integrity` girdisi aliyor
  - `integrity.status === "thin"` ise confidence yuksek olsa bile `automationEligibility` zorla `observe-only` kalir
  - `integrity.status === "partial"` ise otomasyon daha temkinli bicimde `approval-required` seviyesinde tutulur
  - `confidenceFactors` artik `integrity-status` ve `integrity-evidence-score` sinyallerini de tasir
  - son turda `integrity-coherence-score` ve `integrity-contradictions` de confidenceFactors icine eklendi
  - contradiction sayisi `>= 2` ise automation zorla `observe-only`; `== 1` ise en az `approval-required` seviyesinde tutulur
- `platform-data` artik integrity verisini `SystemSummary` ve `CognitiveSummary` icine tasiyor
- ayni integrity katmani customer-mode app kopyasina da senkronlandi
- kurulum scriptleri artik session log uretiyor:
  - yeni helper: `/Users/bariskockar/Desktop/bilet-app/project asylum/scripts/install-session.ts`
  - `install-preflight.ts`, `setup-install.ts`, `bootstrap-install.ts`, `install-doctor.ts`, `postinstall-check.ts`
    her calismada `release/install-session.log` icine JSONL oturum kaydi yazar
  - bu degisiklik customer-mode app altindaki `app/scripts/*` kopyasina da senkronlandi
- customer-mode diagnostics export artik loglari da iceriyor:
  - `app/src/app/api/install-diagnostics/route.ts` `install-session.log` ve `launcher-session.log` metinlerini payload'a ekler
  - desktop launcher `src/main.js` artik `release/launcher-session.log` icine `run-step`, `open-dashboard` ve hata oturumlarini yazar
- desktop launcher yeni session-log degisiklikleriyle tekrar paketlendi
  - guncel macOS uygulamasi tekrar bundle kokune kopyalandi:
    `/Users/bariskockar/Desktop/Project-Asylum-Customer-Mode-Kit/release/customer-mode/Project Asylum Installer.app`
  - launcher icinde ayrica `Oturum Loglarini Kaydet` dugmesi eklendi
  - bu dugme `install-session.log` ve `launcher-session.log` icerigini tek `.log` dosyasi olarak masaustune export eder
- Windows saha testi icin hafif paket uretici eklendi:
  - yeni script: `/Users/bariskockar/Desktop/bilet-app/project asylum/scripts/build-windows-pilot-bundle.ts`
  - yeni npm komutu: `npm run bundle:windows-pilot`
  - script yalnizca Windows testinde gerekli customer-mode parcaciklarini toplar:
    - `Project Asylum Installer x64.exe`
    - `Project Asylum Installer.exe`
    - `app/`
    - `config/`
    ...
    - `runtime/windows/`
    - `scripts/`
  - `app/.next/cache` ile `@next/swc-darwin*`, `@next/swc-linux*`, `electron*` gibi Windows testi icin gereksiz agirliklar disarida birakilir
  - script `PROJECT_ASYLUM_WINDOWS_PILOT_TARGET` ile hedef klasoru override edebilir; bu sayede dogrudan USB gibi harici diske yazdirilabilir
  - masaustune cikarma denemesi mevcut cihazda `ENOSPC` ile bloklandi; `/System/Volumes/Data` uzerinde yaklasik `210 MB` bos alan kaldi ve sadece `runtime/windows` klasoru tek basina `218 MB`
  - sonraki temizlik turunda kit icindeki somut bug kaynaklari duzeltildi:
    - `release/customer-mode/app/.env` bundle'dan silindi
    - root ve desktop-launcher altindaki `.DS_Store` dosyalari silindi
    - `runtime/windows/node/*/*.zip` fazlaliklari silindi
    - `build-windows-pilot-bundle.ts` artik gereksiz belge ve test dosyalarini da filtreler:
      - `app/.env`
      - `app/AGENTS.md`
      - `app/CODEX_HANDOFF.md`
      - `app/AI_TRAINING_STRATEGY.md`
      - `app/COGNITIVE_ARCHITECTURE.md`
      - `app/INSTALLATION.md`
      - `app/LICENSE_REVIEW.md`
      - `app/PROJECT_BLUEPRINT.md`
      - `app/THIRD_PARTY_POLICY.md`
      - `app/TRUST_AND_AUTONOMY_MODEL.md`
      - `app/TUBITAK_1812_BIGG_BASVURU_TASLAGI.md`
      - `app/README.md`
      - `app/tests/*`
      - `runtime/windows/**/*.zip`
- bug notu:
    - Windows extractor hatalarinin bir kismi bozuk USB kopyasindan geldi
    - ikinci kismi ise eski zip'lerdeki `__MACOSX` ve macOS metadata artifaktlarindan kaynaklandi
    - bu nedenle yeni temiz arşiv akisi `COPYFILE_DISABLE=1` ve `zip -X` ile ilerletildi
  - macOS installer tarafinda gorulen `10/11` saglik sorununun kok nedeni `app/.next` klasorunun yokluguydu
  - cozum:
    - `release/customer-mode/scripts/install.sh` artik `.next` yoksa otomatik `npm run build` calistiriyor
    - `release/customer-mode/scripts/install.ps1` ayni mantigi Windows icin de tasiyor
    - `release/customer-mode/scripts/launch-installed-app.mjs` de `.next` eksikse uygulamayi baslatmadan once build almaya calisiyor
  - dogrulama:
    - bundle app'te `.next` silinip installer akisi yeniden calistirildi
    - self-check tekrar `11/11` ve `overallStatus: pass` dondurdu
- dogrulama:
  - canonical repo `npm test` -> `20/20`
  - customer-mode app `npm test` -> `20/20`
  - customer-mode app `npm run build` gecti; sadece bilinen Next `lockfile missing swc dependencies` uyarisi kaldi
  - desktop launcher syntax kontrolu gecti
  - yeni `.app` build ciktisindan bundle kokune tasindi
  - not: `electron-builder` DMG asamasinda `hdiutil` nedeniyle hata verdi; bu `.app` kullanimini engellemiyor
...
- `src/app/components/dashboard-client.tsx` kilitli adımlarda:
  ...
  yazar
- yeni script: `/scripts/install-preflight.ts`
  ...
  - OS bazlı guidance
  üretir
- `scripts/install.sh` ve `scripts/install.ps1` artık customer mode preflight çalıştırır; Node/npm yoksa kurulum zincirine girmeden anlaşılır hata verir
- desktop delivery kit içinde `install.sh` için kritik düzeltme yapıldı:
  bundled macOS/Linux runtime path'i önce yanlış klasörü arıyordu; düzeltildi ve PATH'ten bağımsız testte bundled Node ile akış doğrulandı
- `package.json` içine `install:preflight` eklendi
- `INSTALLATION.md` customer mode kavramı ve tüm OS'ler için ortak kurulum mantığıyla güncellendi
...
- desktop Electron `.app` build sürecinde mac signing adımı uzun sürebiliyor; buna rağmen `.app` çıktısı oluşuyor ve root bundle içine kopyalanabiliyor
