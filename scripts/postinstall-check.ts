...
appendInstallSessionLog({
  step: "postcheck",
  event: checks.every((check) => check.ok) ? "completed" : "failed",
  level: checks.every((check) => check.ok) ? "info" : "error",
  payload: {
    okChecks: checks.filter((check) => check.ok).map((check) => check.id),
