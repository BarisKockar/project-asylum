...
appendInstallSessionLog({
  ...
  payload: {
    ...
    executionStorePath,
    preferredLogSource:
      platformProfile.logSources.find((source) => source.preferred)?.label ?? null,
    readableLogSourceCount: platformProfile.logSources.filter(
      (source) => source.exists && source.readable !== false
    ).length
  }
});
