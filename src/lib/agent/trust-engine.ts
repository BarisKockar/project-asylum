...
export function buildTrustAssessment(input: {
  ...
  risks: ExecutionRisks;
  decision: ExecutionDecision;
  integrity?: PromptExecutionReport["integrity"];
  ...
  const integrityPenalty =
    input.integrity?.status === "thin"
      ...
      : input.integrity?.status === "partial"
        ? 0.08
        : 0;
  const coherencePenalty = clamp(
    ((1 - (input.integrity?.coherenceScore ?? 1)) * 0.18) +
      ((input.integrity?.contradictionCount ?? 0) * 0.04),
    0,
    0.24
  );
  ...
  return {
    ...
    confidenceFactors: [
      ...
      `integrity-status=${input.integrity?.status ?? "unknown"}`,
      `integrity-evidence-score=${(input.integrity?.evidenceScore ?? 0).toFixed(2)}`,
      `integrity-coherence-score=${(input.integrity?.coherenceScore ?? 0).toFixed(2)}`,
      `integrity-contradictions=${input.integrity?.contradictionCount ?? 0}`,
