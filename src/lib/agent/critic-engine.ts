...
export function buildCriticTrace(
  ...
  if (contradictions.length > 0) {
    riskFlags.push("evidence-coherence-review");
  }
  ...
  return {
      ...
      verdict === "approve"
        ...
        : `Critic ek doğrulama istiyor. Öncelikli odak: ${focusHypothesis?.title ?? "en kritik hipotez"}${
            contradictions.length > 0
              ? ` ve contradiction kaynaklari (${contradictions.join(", ")})`
              : ""
          }.`,
    riskFlags,
    policyMatches: [],
    recommendedAction:
      verdict === "approve"
        ...
        : contradictions.length > 0
          ? `${focusHypothesis?.title ?? "Öncelikli hipotez"} için daha güçlü kanıt topla, contradiction kaynaklarını çöz ve policy gate'i yeniden değerlendir.`
