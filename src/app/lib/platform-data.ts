...
export type CognitiveSummary = {
  ...
  integrity: {
    ...
    contradictionExplanations: string[];
    summary: string;
  };
  confidence: number;
  signals: string[];
...
const selfCheckReportPath = path.join(
  process.cwd(),
  "..",
  "release",
...
function describeContradictionGroup(group: {
  ...
  const joinedItems = group.items.join(", ");

  if (group.category === "evidence-gap") {
    return `Kanıt boşluğu: ${joinedItems}. Sistem bazı sinyalleri gördü ama bunları destekleyen yeterli risk/evidence eşleşmesi oluşmadı.`;
  }

  if (group.category === "risk-decision-mismatch") {
    return `Risk-karar uyumsuzluğu: ${joinedItems}. Üretilen risk seviyesi ile karar statüsü birbiriyle tam örtüşmüyor.`;
  }
...
function readInstallationState() {
  ...
  const selfCheck = fs.existsSync(selfCheckReportPath)
    ? (() => {
        try {
          const parsed = JSON.parse(fs.readFileSync(selfCheckReportPath, "utf8")) as {
            overallStatus?: string;
          ...
          return {
            ...
            failedChecks: Math.max(checks.length - passedChecks, 0),
            popupReady: parsed.popupReady === true,
            launchUrl:
              typeof parsed.launchUrl === "string" ? parsed.launchUrl : null
          };
        } catch {
          return {
            available: false,
  ...
  const sourcePaths =
    platformProfile?.logSources.map((source) => ({
      label: source.label,
      path: source.path,
      status: source.exists && source.readable !== false
        ? ("detected" as const)
        : ("pending" as const)
    })) ?? [
      {
...
export function getSystemSummary(): SystemSummary {
  ...
  return {
    ...
    integrity: {
      status: latestExecutionReport()?.integrity?.status ?? "thin",
      evidenceScore: latestExecutionReport()?.integrity?.evidenceScore ?? 0,
      coherenceScore: latestExecutionReport()?.integrity?.coherenceScore ?? 0,
      coverageScore: latestExecutionReport()?.integrity?.coverageScore ?? 0,
      taskCompletionScore:
        latestExecutionReport()?.integrity?.taskCompletionScore ?? 0,
      pilotReady: latestExecutionReport()?.integrity?.pilotReady ?? false,
      contradictionCount:
        latestExecutionReport()?.integrity?.contradictionCount ?? 0,
      summary:
        latestExecutionReport()?.integrity?.summary ??
        "Kanıt bütünlüğü henüz hesaplanmadı."
    },
...
export function getCognitiveSummary(): CognitiveSummary {
  ...
  return {
    ...
    integrity: {
      status: latestReport?.integrity?.status ?? "thin",
      evidenceScore: latestReport?.integrity?.evidenceScore ?? 0,
      coherenceScore: latestReport?.integrity?.coherenceScore ?? 0,
      missingKinds: latestReport?.integrity?.missingKinds ?? [],
      pilotReady: latestReport?.integrity?.pilotReady ?? false,
      contradictionCount: latestReport?.integrity?.contradictionCount ?? 0,
      contradictionGroups: latestReport?.integrity?.contradictionGroups ?? [],
      contradictions: latestReport?.integrity?.contradictions ?? [],
      contradictionExplanations: (
        latestReport?.integrity?.contradictionGroups ?? []
      ).map((group) => describeContradictionGroup(group)),
      summary:
        latestReport?.integrity?.summary ??
        "Kanıt bütünlüğü henüz hesaplanmadı."
    },
